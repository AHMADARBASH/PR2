...
class HotelsRepository {
  ...
  Future<List<Hotel>> searchForHotels(name) async {
    final url = Uri.parse('${baseURL}/hotels/by_name?name=$name');
    final response = await http.get(url);
    if (response.statusCode != 200) {
      throw HTTPException(message: json.decode(response.body));
    }
  ...
  Future<List<Hotel>> searchForHotelsByPrice(sPrice,ePrice) async {
    final url = Uri.parse('${baseURL}/hotels/by_room_price?start_price=$sPrice&end_price=$ePrice');
    final response = await http.get(url);
    if (response.statusCode != 200) {
      throw HTTPException(message: json.decode(response.body));
    }
